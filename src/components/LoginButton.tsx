...
const LoginButton: React.FC<LoginButtonProps> = ({ provider, className = '', dataLanguage }) => {
  ...
  console.log("data language: ", dataLanguage?.dataAction);
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={loading}
        className={`
          flex items-center bg-white dark:bg-gray-900 border border-gray-300 rounded-lg shadow-md px-6 py-2 text-sm font-medium text-gray-800 dark:text-white hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-all gap-2
      ...
      >
        {loading ? (
          ...
          <span className=" left-4">{providerInfo.icon}</span>
        )}
